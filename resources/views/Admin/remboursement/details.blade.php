...
<div class="p-8 bg-gray-50 min-h-screen">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <a href="{{ route('admin.remboursement.index') }}" class="inline-flex items-center gap-2 text-sm font-bold text-orange-500 hover:text-orange-600 transition mb-3">
        ...
        <div class="bg-white px-5 py-3 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div class="w-16 h-16 relative">
                <svg class="w-full h-full" viewBox="0 0 36 36">
                    ...
                    <path class="text-orange-500" stroke-width="3" stroke-dasharray="{{ $demande->pourcentage }}, 100" stroke-linecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                </svg>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white rounded-[2rem] p-8 shadow-xl shadow-gray-200/40 border border-gray-100 relative overflow-hidden">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <p class="text-2xl font-black text-gray-900">{{ number_format($demande->montant_souhaite, 0, ',', ' ') }} <span class="text-xs">F</span></p>
                    </div>
                    
                    <div class="pt-4 border-t border-gray-50">
                        <p class="text-sm text-green-500 mb-1 font-bold">Déjà Remboursé</p>
                        <p class="text-2xl font-black text-green-600">{{ number_format($demande->paye, 0, ',', ' ') }} <span class="text-xs">F</span></p>
                    </div>

                    <div class="pt-4 border-t border-gray-50">
                        <p class="text-sm text-red-400 mb-1 font-bold">Reste à percevoir</p>
                        <p class="text-3xl font-black text-red-500">{{ number_format($demande->restant, 0, ',', ' ') }} <span class="text-xs font-bold uppercase">FCFA</span></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="flex flex-col">
        ...
        <span class="text-[10px] font-black text-orange-500 uppercase tracking-widest mb-1">
            Échéance {{ $loop->iteration }} / {{ $demande->echeances->count() }}
        </span>
        
        <div class="text-xl font-black text-gray-900 leading-none">
            {{ number_format($echeance->montant, 0, ',', ' ') }} <small class="text-xs">F</small>
        </div>
            ...
            Ref: #ECH-{{ $echeance->id }}
                            ...
                            <td class="p-6">
                                @php
                                    ...
                                    $isEnRetard = \Carbon\Carbon::parse($echeance->date_echeance)->isPast() && $statut !== 'payé';
                                @endphp

                                @if($statut === 'payé' || $statut === 'valide')
                                    <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-green-50 text-green-600 border border-green-100">
                                        <span class="relative flex h-2 w-2">
                                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                                            <span class="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
                                        </span>
                                        <span class="text-[10px] font-black uppercase tracking-widest">Versement Reçu</span>
                                    </div>
                                @elseif($isEnRetard)
                                    <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-red-50 text-red-600 border border-red-100">
                                        <i class="fas fa-exclamation-circle text-[10px]"></i>
                                        <span class="text-[10px] font-black uppercase tracking-widest">En Retard</span>
                                    </div>
                                @else
                                    <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100">
                                        <i class="far fa-clock text-[10px]"></i>
                                        <span class="text-[10px] font-black uppercase tracking-widest">À Venir</span>
                                    </div>
                                @endif
                            </td>
                        </tr>
