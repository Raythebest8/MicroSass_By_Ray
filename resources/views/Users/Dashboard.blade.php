...
<div class="px-4 md:px-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border-l-4 border-indigo-500 flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-500 uppercase dark:text-gray-400">Total Accordé</p>
                <p class="text-2xl font-bold text-gray-800 mt-1 dark:text-white" id="stat-total">
                    {{ number_format($totalAccorde, 0, ',', ' ') }} <span class="text-sm">FCFA</span>
                </p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border-l-4 border-emerald-500 flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-gray-800 mt-1 dark:text-white" id="stat-paid">
                    {{ number_format($totalRembourse, 0, ',', ' ') }} <span class="text-sm">FCFA</span>
                </p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border-l-4 border-amber-500 flex items-center justify-between">
            <div class="text-3xl font-extrabold mt-2 text-amber-600">
    @if($prochaineEcheance)
        {{ number_format($prochaineEcheance->montant_total, 0, ',', ' ') }} FCFA
    @else
        0 FCFA
    @endif
</div>
<p class="text-xs text-gray-500 mt-1">
    {{ $prochaineEcheance ? \Carbon\Carbon::parse($prochaineEcheance->date_prevue)->format('d M Y') : 'Aucun paiement à venir' }}
</p>
            ...
            <div class="flex-1 overflow-y-auto custom-scroll pr-2 max-h-[300px]">
                <ul class="space-y-4">
                    <li class="flex items-start pb-4 border-b border-gray-100 dark:border-gray-700 last:border-0">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-800 dark:text-white">Demande  </p>
                            <p class="text-xs text-gray-500 dark:text-gray-400"></p>
                            <p class="text-sm font-bold text-green-600 dark:text-green-400 mt-1"></p>
                        </div>
                    </li>
                    <li class="flex items-start pb-4 border-b border-gray-100 dark:border-gray-700 last:border-0">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-4">
                <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                    ...
                    <p class="text-lg font-bold text-indigo-600 dark:text-indigo-400" id="detail-rate">{{ $detail->taux_interet ?? '0' }} %</p>
                </div>
                <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                    <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">Mensualité</p>
                    <p class="text-lg font-bold text-indigo-600 dark:text-indigo-400" id="detail-monthly">{{ number_format($detail->mensualite ?? 0, 0, ',', ' ') }} FCFA</p>
                </div>
                <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                    <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">Prochaine Échéance</p>
                    <p class="text-lg font-bold text-amber-600 dark:text-amber-400" id="detail-next">{{ $detail->prochaine_echeance ?? 'Aucune' }}</p>
                </div>
